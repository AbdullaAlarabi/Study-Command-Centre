...
const outputDir = resolve('reports/phase7-qa')
...
async function shot(page: Page, name: string) {
  for (const [suffix, width, height] of [['mobile', 390, 844], ['desktop', 1440, 1000]] as const) {
    await page.setViewportSize({ width, height })
    await page.screenshot({ path: resolve(outputDir, `${name}-${suffix}.png`), fullPage: true })
  }
  mark(`${name} renders at 390×844 and 1440×1000`)
}
...
async function completeChapterQuiz(page: Page, userId: string, unitId: string, pass: boolean) {
...
async function cleanup(studentId: string) {
  ...
  if (error) throw error
  console.log(`CLEANUP removed ${ids.length} QA attempts and their linked reviews/activity.`)
}
...
async function run() {
  ...
  if (chapters.length !== 3 || !revision || !mockOne) throw new Error('Could not resolve the MKT112 midterm path.')
  ...
  try {
    ...
    await completeChapterQuiz(studentPage, student.id, chapters[0].id, false)
    await shot(studentPage, 'chapter-quiz-failed')
    await studentPage.getByRole('button', { name: 'Try a new selection' }).click()
    await completeChapterQuiz(studentPage, student.id, chapters[0].id, true)
    ...
    await coachPage.goto(pathUrl(`/coach/attempts/${mockAttemptId}`), { waitUntil: 'networkidle' })
    ...
    const report = { qaStartedAt, completedAt: new Date().toISOString(), baseUrl, viewports: ['390x844', '1440x1000'], checks, screenshotCount: checks.filter((check) => check.includes('renders at')).length * 2, temporaryAttempts: createdAttemptIds.size }
